...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    P_INFO("Creating logical device...");
...
void vulkan_device_destroy(vulkan_context* context) {
    ...
    P_INFO("Releasing physical device resources...");
...
b8 
vulkan_device_detect_depth_format(vulkan_device* device) {
    // Format candidates
...
b8 select_physical_device(vulkan_context* context) {
    ...
    if (physical_device_count == 0) {
        P_FATAL("No devices which support Vulkan were found.");
        return FALSE;
    }
    ...
    if (!context->device.physical_device) {
        P_ERROR("No physical devices were found which meet the requirements.");
        return FALSE;
    }
...
b8 physical_device_meets_requirements(
    ...
    if (
        ...
        P_TRACE("Graphics Family Index: %i", out_queue_info->graphics_family_index);
        P_TRACE("Present Family Index:  %i", out_queue_info->present_family_index);
        P_TRACE("Transfer Family Index: %i", out_queue_info->transfer_family_index);
        P_TRACE("Compute Family Index:  %i", out_queue_info->compute_family_index);
        ...
        if (requirements->device_extension_names) {
            ...
            if (available_extension_count != 0) {
                ...
                for (u32 i = 0; i < required_extension_count; ++i) {
                    ...
                    if (!found) {
                        P_INFO("Required extension not found: '%s', skipping device.", requirements->device_extension_names[i]);
        ...
        if (requirements->sampler_anisotropy && !features->samplerAnisotropy) {
            P_INFO("Device does not support samplerAnisotropy, skipping.");
            return FALSE;
        }
