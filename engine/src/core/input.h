...
// Types of buttons we are supporting
// Currently just the mouse click buttons
...
/// NOTE: these are Windows platform specific. We need to translate within Linux platform
typedef enum keys {
...
// Keyboard input
P_API b8 input_is_key_down(keys key);
P_API b8 input_is_key_up(keys key);
P_API b8 input_was_key_down(keys key);
P_API b8 input_was_key_up(keys key);

void input_process_key(keys key, b8 pressed);

// Mouse input
P_API b8 input_is_button_down(buttons button);
P_API b8 input_is_button_up(buttons button);
P_API b8 input_was_button_down(buttons button);
P_API b8 input_was_button_up(buttons button);
P_API void input_get_mouse_positions(i32* x, i32* y);
P_API void input_get_previous_mouse_positions(i32* x, i32* y);

void input_process_button(buttons button, b8 pressed);
void input_processmouse_move(i16 x, i16 y);
